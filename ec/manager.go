...
func (m *Manager) encode(c_reader chan data_chunk, c_writers []chan byte){
	k, n := int(m.k), int(m.n)
...
func (m *Manager) Decode(shard_paths []string, outpath string){
	...
	c_writer := make(chan byte, 2*int(m.n)) //c via which decoded data is sent to writeFile()
...
func (m *Manager) Encode(inpath string) ([]string, error) {
	...
	for i := range c_writers {
		c_writers[i] = make(chan byte, 3*int(m.n))
	...
	go readFile(inpath, c_reader) //make routine which will read file
